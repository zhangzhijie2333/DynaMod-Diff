...
class MvtecSemanticDataset(torch.utils.data.Dataset):
    ...
    def _random_resized_crop_pair(self, image: Image.Image, mask: Image.Image, anomaly_present: bool):
    ...
    def _apply_affine_pair(self, image: Image.Image, mask: Image.Image, anomaly_present: bool):
    ...
    def _paired_spatial_augment(self, image: Image.Image, mask: Image.Image):
        ...
        if random.random() < self.paired_aug_crop_prob:
            image, mask = self._random_resized_crop_pair(image, mask, anomaly_present)

        image, mask = self._apply_affine_pair(image, mask, anomaly_present)
        return image, mask
    ...
    def __getitem__(self, index):
        ...
        if self.paired_aug_enable:
            image, sem = self._paired_spatial_augment(image, sem)
