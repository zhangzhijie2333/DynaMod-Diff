...
class GatedSelfAttentionDense(nn.Module):
    ...
    def forward(self, x, objs):
        ...
        N_obj = objs.shape[1]
        cat_tokens = self.norm1(torch.cat([x,objs],dim=1))

        if self.capture_attention:
            attn_out, attn_map = self.attn(cat_tokens, return_attn=True)
            visual_to_obj = attn_map[:, :, :N_visual, N_visual:]
            side = int(math.sqrt(N_visual))
            hw = (side, side) if side * side == N_visual else (None, None)
            self.last_attention_map = visual_to_obj
            self.last_attention_meta = {
                "n_visual": N_visual,
                "n_obj": N_obj,
                "visual_hw": hw,
                "batch_size": B,
            ...
            attn_out = self.attn(cat_tokens)
            self.last_attention_map = None
            self.last_attention_meta = None

        x = x + self.scale*torch.tanh(self.alpha_attn) * attn_out[:,0:N_visual,:]
        x = x + self.scale*torch.tanh(self.alpha_dense) * self.ff( self.norm2(x) )
...
class GatedSelfAttentionDense2(nn.Module):
    ...
    def forward(self, x, objs):
        ...
        if self.capture_attention:
            attn_out, attn_map = self.attn(self.norm1(torch.cat([x,objs],dim=1)), return_attn=True)
            self.last_attention_map = attn_map[:, :, :N_visual, N_visual:]
            self.last_attention_meta = {
                "n_visual": N_visual,
                "n_obj": N_ground,
                "visual_hw": (size_v, size_v),
                "batch_size": B,
            }
        else:
            attn_out = self.attn(self.norm1(torch.cat([x,objs],dim=1)))
            self.last_attention_map = None
            self.last_attention_meta = None

        # select grounding token and resize it to visual token size as residual 
        out = attn_out[:,N_visual:,:]
...
#        return checkpoint(self._forward, (x, context, objs), self.parameters(), self.use_checkpoint)
        if self.use_checkpoint and x.requires_grad:
            return checkpoint.checkpoint(self._forward, x, context, objs,use_reentrant=False)
        else:
