...
class DecoderWeightsMaskGenerator(nn.Module):
    ...
    def forward(self, weight_shape, sample, highfreq_cond=None, grounding_extra_input=None, layer_name=None):
        ...
        if self.use_sem_cond and grounding_extra_input is not None:
            if grounding_extra_input.dim() != 4:
                raise ValueError(
                    f"grounding_extra_input must be 4D [B,C,H,W], got {tuple(grounding_extra_input.shape)} for layer {layer_name}"
                )
            if grounding_extra_input.shape[1] != self.sem_in_dim:
                raise ValueError(
                    f"grounding_extra_input channel mismatch: expect {self.sem_in_dim}, got {grounding_extra_input.shape[1]} for layer {layer_name}"
                )
            if tuple(grounding_extra_input.shape[-2:]) != tuple(target_hw):
                grounding_extra_input = F.adaptive_avg_pool2d(grounding_extra_input, target_hw)
            sem_cond = self.proj_sem(grounding_extra_input).to(dtype=x.dtype, device=x.device)

        x = x + hf_cond + sem_cond
...
def get_decoder_conv_hook_fn(layer_name=None):
    def hook_fn(module, input, output):
        ...
        if grounding_extra_input is not None and grounding_extra_input.size(0) != batch_size:
            if grounding_extra_input.size(0) == 1:
                grounding_extra_input = grounding_extra_input.expand(batch_size, -1, -1, -1)
            else:
                raise ValueError(
                    f"grounding_extra_input batch mismatch for layer {layer_name}: cond batch={grounding_extra_input.size(0)}, input batch={batch_size}"
                )
...
class Decoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        decoder_mask_use_sem_cond=False,
        ...
        **ignorekwargs,
        ...
        self.decoder_mask_use_sem_cond = bool(decoder_mask_use_sem_cond)
        self.decoder_mask_sem_in_dim = int(decoder_mask_sem_in_dim)
    ...
    def _build_decoder_mask_modules(self):
        ...
        for bi, block in enumerate(self.up):
            ...
            mg = DecoderWeightsMaskGenerator(
                ...
                sem_in_dim=self.decoder_mask_sem_in_dim,
                use_sem_cond=self.decoder_mask_use_sem_cond,
            )
        ...
        if self.debug_decoder_mask or self.decoder_mask_build_verbose:
            print(
                ...
                f"use_highfreq={self.decoder_mask_use_highfreq_cond} "
                f"use_sem={self.decoder_mask_use_sem_cond} "
                f"use_affine={self.decoder_mask_use_affine}"
            )
    ...
    def _register_decoder_conv_ctx(
        self,
        module,
        sample,
        mask_generator,
    ...
    def forward(self, z, mask_cond_img=None, mask_cond_highfreq=None, grounding_extra_input=None):
        ...
        need_sem_for_decoder = use_decoder_mask and self.decoder_mask_use_sem_cond
        ...
        for i_level in reversed(range(self.num_resolutions)):
            if use_decoder_mask:
                ...
                stage_sem = None
                if need_sem_for_decoder and grounding_extra_input is not None:
                    if grounding_extra_input.dim() != 4:
                        raise ValueError(
                            f"grounding_extra_input must be 4D [B,C,H,W], got {tuple(grounding_extra_input.shape)}"
                        )
                    sem0 = grounding_extra_input.to(device=h.device)
                    if not torch.is_floating_point(sem0):
                        sem0 = sem0.float()
                    else:
                        sem0 = sem0.to(dtype=h.dtype)

                    if tuple(sem0.shape[-2:]) == target_hw:
                        stage_sem = sem0
                    elif target_hw in sem_cache:
                        stage_sem = sem_cache[target_hw]
                    else:
                        stage_sem = F.interpolate(sem0, size=target_hw, mode="nearest")
                        sem_cache[target_hw] = stage_sem
                ...
                self._register_decoder_conv_ctx(
                    ...
                    grounding_extra_input=stage_sem,
                )
